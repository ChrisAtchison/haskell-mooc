...
add b (Set xs) = case member b (Set xs) of
  True  -> (Set xs)
  False -> Set (sort ([b] ++ xs))
...
data State = Start | Eggs | F | S | FS | Mixed | Error | Finished
  deriving (Eq,Show)

step :: State -> Event -> State
step Start AddEggs = Eggs
step Eggs AddFlour = F
step Eggs AddSugar = S
step S AddFlour = FS
step F AddSugar = FS
step FS Mix = Mixed
...
passwordAllowed p (MinimumLength n)
  | length p < n = False
  | otherwise = True
...
passwordAllowed p (And a b) = (passwordAllowed p a) && (passwordAllowed p b)
passwordAllowed p (Or a b) = (passwordAllowed p a) || (passwordAllowed p b)
