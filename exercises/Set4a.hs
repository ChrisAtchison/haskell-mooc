...
longest :: Ord a => [[a]] -> [a]
longest xs = head $ reverse $ sortBy (comparing length) $ reverse $ sortBy (comparing head) xs
...
incrementKey _ []           = []
incrementKey z ((y,x):xs)
  | y == z      = (y, x+1) : incrementKey z xs
  | otherwise   = (y, x)   : incrementKey z xs
...
winner scores player1 player2 = if (Map.findWithDefault 0 player1 scores) 
                                   >= 
                                   (Map.findWithDefault 0 player2 scores)
                                then player1
                                else player2
...
freqs xs = Map.fromList [(a, (length $ filter (==a) xs)) | a <- xs ]
...
transfer from to amount bank
  ...
  | otherwise           = case Map.lookup from bank of
                          ...
                          Just x  -> if x < amount 
                                     then bank
                                     else Map.adjust (\x -> x + amount) to (Map.adjust (\x -> x - amount) from bank)
...
maxIndex arr = go (assocs arr)
  where go [a]          = fst a
        go (a : b : xs)  = if snd a > snd b
                          then go (a : xs)
                          else go (b : xs)
