...
allValues condition (Node a l r)
  | condition a = True && (allValues condition l) && (allValues condition r)
  | otherwise   = False
...
isOrdered (Node a l r)  = (allValues (<a) l) 
                          && (allValues (>a) r) 
                          && (isOrdered l) 
                          && (isOrdered r)
...
set _ _ Empty = Empty
set (x:xs) val (Node a l r)
  | x == StepL = (Node a (set xs val l) r)
...
search val (Node a l r) = find val (Node a l r) []
  where find val Empty path = Nothing
        find val (Node a l r) path
          | val == a  = Just path
          | otherwise = case (find val l (path ++ [StepL])) of
                          Nothing -> find val r (path ++ [StepR])
                          Just x  -> Just x
