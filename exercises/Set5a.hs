...
up p = MkPosition (x p) (1 + y p)

-- right increases the x value of a position by one
right :: Position -> Position
right p = MkPosition (1 + x p) (y p)
...
fromNat :: Nat -> Int
fromNat Zero          = 0
fromNat (PlusOne xs)  = 1 + fromNat xs
...
prettyPrint b = reverse $ prettyPrint' b

prettyPrint' :: Bin -> String
prettyPrint' End     = []
prettyPrint' (O xs)  = '0' : prettyPrint' xs
prettyPrint' (I xs)  = '1' : prettyPrint' xs

fromBin :: Bin -> Int
fromBin b = go 1 b
  where go val End    = 0
        go val (O xs) = 0 + go (val * 2) xs
        go val (I xs) = val + go (val * 2) xs

toBin :: Int -> Bin
toBin 0 = O End
toBin 1 = I End
toBin n = case rem n 2 of
  1 -> I (toBin (quot n 2))
  0 -> O (toBin (quot n 2))