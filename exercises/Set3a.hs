...
firstHalf x
  | (length x) == 1 = x
  | otherwise       = take (div ((length x) + 1) 2) x
...
capitalize [] = []
capitalize ss = unwords . map (\x -> toUpper(head x) : tail x) $ words ss
...
joinToLength n list = [a ++ b | a <- list, b <- list, length (a ++ b) == n]
...
interpreter' commands x y
  | commands == []  = []
  | otherwise       = interpreter'' commands x y

interpreter'' commands x y = case head commands of
  "up"      -> interpreter' (tail commands) x (y + 1)
  "down"    -> interpreter' (tail commands) x (y - 1)
  "left"    -> interpreter' (tail commands) (x - 1) y
  "right"   -> interpreter' (tail commands) (x + 1) y
  "printX"  -> show x : interpreter' (tail commands) x y
  "printY"  -> show y : interpreter' (tail commands) x y
