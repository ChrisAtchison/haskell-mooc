...
indexDefault xs i def = go xs i
  where go [] _     = def
        go (x:xs) 0 = x
        go (x:xs) i = indexDefault xs (i - 1) def
...
sorted xs = go xs
  where go []       = True
        go [x]      = True
        go (x:y:xs) = if x > y then False else sorted (y:xs)
...
merge xs ys = go xs ys
  where go [] _          = ys
        go _ []          = xs
        go (x:xs) (y:ys) = if x < y then x : merge xs (y:ys) else y : merge (x:xs) ys
...
map2 f as bs = go as bs
  where go [] _           = []
        go _ []           = []
        go (a:as) (b:bs)  = f a b : map2 f as bs
